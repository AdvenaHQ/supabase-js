...
} from "@supabase/ssr/dist/main/types";
import type {
    PostgrestError,
 ...
 */
export type UseSupabaseCacheProvidersOptions =
    | UseSupabaseCacheSupacacheProviderOptions
    | UseSupabaseCacheUpstashRedisProviderOptions
    | UseSupabaseCacheRedisProviderOptions;
/**
...
export type UseSupabaseOptions = {
    ...
    cache?: {
         ...
         */
        provider:
            | UseSupabaseCacheSupacacheProviderOptions["provider"]
            | UseSupabaseCacheUpstashRedisProviderOptions["provider"]
            | UseSupabaseCacheRedisProviderOptions["provider"];
 ...
 */
export type SupabaseClientOptionsType =
    | SupabaseClientOptions<"public">
    | SupabaseClientOptions<"realtime">
    | SupabaseClientOptions<unknown>
    // biome-ignore lint/suspicious/noExplicitAny: Any is required for the Supabase client options
 ...
 */
export type SupabaseClientServerOptionsType = SupabaseClientOptionsType &
    Partial<{
 ...
 */
export type SupabaseClientBrowserOptionsType = SupabaseClientOptionsType &
    Partial<{
        cookieOptions?: CookieOptionsWithName;
        cookies?: CookieMethodsBrowser; // Use browser-specific cookies
        cookieEncoding?: "raw" | "base64url";
    }>;
...
export type SupabaseClient<
    Database = GenericDatabase, // or Record<string, any>
    ...
    Schema extends GenericSchema = Database[SchemaName] extends GenericSchema
        ...
        : // biome-ignore lint/suspicious/noExplicitAny: Any is required for the Supabase client options
          any,
    // biome-ignore lint/suspicious/noExplicitAny: Any is required for the Supabase client options
    SchemaResponse extends GenericSchema = any,
> = Omit<_SupabaseClient<Database, SchemaName, SchemaResponse>, "cookies">;
...
export type ExtendedSupabaseClient<
    ...
    Schema extends GenericSchema = Database[SchemaName] extends GenericSchema
        ? Database[SchemaName]
        : GenericDatabase,
> = Omit<SupabaseClient<Database, SchemaName, Schema>, "from"> & {
    from: <
 ...
 */
export type SupabaseClientConstructorServerFetchOptions =
...
export type GenericDatabase =
    // biome-ignore lint/suspicious/noExplicitAny: `any` is used here for maximum flexibility in defining the database schema.
    any;
 ...
 */
export type UseSupabaseGenerics<
    Database = GenericDatabase,
    SchemaName extends string & keyof Database = "public" extends keyof Database
        ? "public"
        : string & keyof Database,
...
> extends PostgrestQueryBuilder<Schema, Relation, RelationName, Relationships> {
    // Optional property to specify cache expiration time in seconds
    expireAfterSeconds?: number;
    // Method to set the cache expiration time and return the modified query builder
    cache: (
        expireAfterSeconds: number,
    ) => ExtendedQueryBuilder<Schema, Relation, RelationName, Relationships>;
    // Override the select method to include caching logic
    // biome-ignore lint/suspicious/noExplicitAny: This is a generic type that is used to define the schema of the database
    select: <Query extends string = "*", ResultOne = any>(
        columns?: Query,
        options?: {
            head?: boolean;
